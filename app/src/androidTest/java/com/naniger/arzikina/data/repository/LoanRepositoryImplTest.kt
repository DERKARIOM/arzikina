 ...
 * (voir la doc de [com.naniger.arzikina.domain.repository.LoanRepository]).
 */
...
class LoanRepositoryImplTest {
    ...
    fun findLoanIdForTransaction_retrouveLePretViaSonDecaissementOuUnRemboursement() = runBlocking {
        ...
        val normalTransactionId = database.transactionDao().upsert(
            com.naniger.arzikina.data.local.entity.TransactionEntity(
                userId = userId,
                amount = 500L,
                type = com.naniger.arzikina.domain.model.TransactionType.EXPENSE,
                accountId = accountId,
