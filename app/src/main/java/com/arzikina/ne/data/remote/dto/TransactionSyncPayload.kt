 ...
 * [com.arzikina.ne.data.repository.TransactionSyncEnqueuer] (voir sa KDoc pour la stratégie de
 * résolution et le filet de sécurité si un `syncId` référencé est étonnamment absent).
 *
 * Champs volontairement EXCLUS :
 * - [com.arzikina.ne.data.local.entity.TransactionEntity.receiptId] : les reçus ne sont pas encore
 *   synchronisés — même raisonnement que `CardSecretEntity` pour `Account` (voir la KDoc de tête de
 *   `AccountSyncPayload.kt`).
 * - [com.arzikina.ne.data.local.entity.TransactionEntity.receiptPhotoUri] : chemin de fichier LOCAL
